...
test.beforeEach(async ({ page }) => {
  ...
  await page.goto(
    `/channels/${encodeURIComponent(fixture.generalChannel.name)}`,
  );
  await expect(page.getByTestId("channel-title")).toContainText(
    fixture.generalChannel.name,
  );
});
...
test("creates and manages a retrospective board", async ({ page }) => {
  ...
  await wentWell
    .locator(".retro-item")
    .filter({ hasText: "The release process stayed smooth" })
    .dragTo(backlog);
...
test("shows optional linked work URL for backlog ideas", async ({ page }) => {
  ...
  await board
    ...
    .click();
  const ideaModal = page.locator(".retro-idea-modal");
  const link = ideaModal.locator(
    'input[placeholder="Paste a link (optional)"]',
  );
