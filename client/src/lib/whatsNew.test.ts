...
describe("What's new release content", () => {
  it("matches authored content to the desktop version", () => {
    assert.equal(getWhatsNewRelease("0.42.0")?.id, "0.42.0");
    assert.deepEqual(
      getWhatsNewRelease("0.42.0")?.items.map(({ title }) => title),
      ["Thread timestamps", "UI improvements", "Bug fixes"],
    );
    ...
    assert.deepEqual(
      ...
      ["Also send to channel", "Seamless Echo updates"],
    );
  ...
  it("provides the upcoming release in browser preview mode", () => {
    ...
    assert.deepEqual(getWhatsNewReleases("0.39.0", true).map(({ id }) => id), ["0.42.0", "0.41.0", "0.40.0", "0.39.0"]);
  });

  it("returns all authored releases for a known desktop version", () => {
    assert.deepEqual(getWhatsNewReleases("0.40.0").map(({ id }) => id), ["0.42.0", "0.41.0", "0.40.0", "0.39.0"]);
  });
