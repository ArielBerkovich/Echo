...
export function normalizeChannelNameInput(value) {
  return String(value || "")
    .trim()
    .replace(/^#/, "")
    .toLowerCase()
...
export function normalizeEmojiNameInput(value) {
  return String(value || "")
    .trim()
    .replace(/^:|:$/g, "")
    .toLowerCase();
...
const loginPasswordInput = z.string().min(1, "Password is required");
  ...
  name: z
    ...
    .pipe(
      z
        ...
        .regex(/^[a-z0-9_-]+$/, "Channel name can only contain lowercase letters, numbers, -, and _")
    ),
...
export function emojiNameSchema(existing = []) {
  ...
  return z
    ...
    .pipe(
      z
        .string()
        .min(2, "Shortcode must be 2-32 characters")
        .max(32, "Shortcode must be 2-32 characters")
        .regex(/^[a-z0-9_-]+$/, "Shortcode can only contain letters, numbers, _ or -")
    )
