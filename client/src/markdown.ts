    ...
    extensions: [
      ...
      },
      {
        ...
        renderer(token) {
          ...
          return `<span class="channel-tag" dir="ltr"${channelId} data-channel-tag="${escapeHtml(token.name)}">#${escapeHtml(token.name)}</span>`;
        },
      },
      {
        ...
        renderer(token) {
          ...
          return `<img class="custom-emoji" src="${token.customUrl}" alt=":${token.code}:" />`;
        },
      },
      {
