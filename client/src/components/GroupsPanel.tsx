...
import { ChevronRightIcon, ContactRoundIcon, LogOutIcon, PlusIcon, SearchIcon, UserMinusIcon, UserPlusIcon, UsersRoundIcon, XIcon } from "lucide-react";
...
export default function GroupsPanel({ onOpenProfile, openGroup = null }) {
  ...
  const filteredGroups = useMemo(() => {
    ...
    const visibleGroups = text ? groups.filter((group) => `${group.name} ${group.handle} ${group.description}`.toLowerCase().includes(text)) : groups;
  ...
  async function createGroup(event) {
    ...
    if (!GROUP_NAME_PATTERN.test(name)) {
      setCreateError("Group names may contain English letters, numbers, spaces, and hyphens only.");
      return;
    }
      ...
      <div className="groups-panel-layout">
        <aside className="groups-panel-list" aria-label="Group directory">
          ...
          <div className="groups-panel-list-body">
            {loadingGroups ? <div className="groups-empty-state"><p>Loading groups…</p></div> : filteredGroups.length ? filteredGroups.map((group) => <button type="button" key={group.id} className={`groups-panel-group${selected?.id === group.id ? " active" : ""}`} aria-current={selected?.id === group.id ? "true" : undefined} onClick={() => setSelected(group)}><span className="groups-panel-group-icon"><ContactRoundIcon size={18} aria-hidden="true" /></span><span className="groups-panel-group-copy"><strong>{group.name}{group.isMember ? <span className="groups-membership-badge">Joined</span> : null}</strong><small>{group.memberCount} {group.memberCount === 1 ? "member" : "members"}</small></span><ChevronRightIcon className="groups-panel-group-arrow" size={16} aria-hidden="true" /></button>) : <div className="groups-empty-state"><span className="groups-empty-state-icon"><ContactRoundIcon size={24} aria-hidden="true" /></span><strong>{query ? "No matching groups" : "No groups yet"}</strong><p>{query ? `No group matches “${query.trim()}”.` : "Create a group to bring people together."}</p></div>}
          </div>
        </aside>
