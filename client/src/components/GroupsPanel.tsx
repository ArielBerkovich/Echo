...
import { ChevronRightIcon, ContactRoundIcon, HashIcon, PlusIcon, SearchIcon, UserPlusIcon, UsersRoundIcon, XIcon } from "lucide-react";
        ...
        <section className="groups-panel-detail" aria-live="polite">
          {!selected ? <div className="groups-detail-empty"><span className="groups-empty-state-icon"><ContactRoundIcon size={26} aria-hidden="true" /></span><strong>Select a group</strong><p>Choose a group to view its people and channels.</p></div> : <>
            ...
            <div className="groups-detail-grid">
              ...
              <section className="groups-detail-section" aria-labelledby="groups-channels-heading"><header className="groups-detail-section-head"><div><h3 id="groups-channels-heading">Channels</h3><p>Channels associated with this group.</p></div></header><div className="groups-channel-list">{selected.channels.length ? selected.channels.map((channel) => <div className="groups-channel-row" key={channel.id}><span className="groups-channel-icon"><HashIcon size={16} aria-hidden="true" /></span><span className="groups-channel-copy"><strong>{channel.name}</strong><small>{channel.type === "private" ? "Private channel" : "Public channel"}</small></span></div>) : <div className="groups-section-empty">No channels associated.</div>}</div></section>
            </div>
