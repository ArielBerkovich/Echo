...
export default function GroupsPanel({ onOpenProfile, openGroup = null }) {
  ...
  async function createGroup(event) {
    ...
    try { const result = await api.createGroup(form); setForm({ name: "", handle: "", description: "" }); setCreating(false); setError(""); await refresh(result.group.id); }
  ...
  return <main className="groups-panel" data-testid="groups-panel" aria-label="Groups">
    ...
    <div className="messages groups-panel-body">{error ? <div className="error" role="alert">{error}</div> : null}
      {creating ? <form className="groups-create-form" onSubmit={createGroup}><strong>Create group</strong><input required maxLength={80} aria-label="Group name" placeholder="Group name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /><input required pattern="[a-z0-9][a-z0-9-]{0,30}[a-z0-9]" maxLength={32} aria-label="Group handle" placeholder="handle, e.g. frontend" value={form.handle} onChange={(event) => setForm({ ...form, handle: event.target.value.toLowerCase() })} /><textarea maxLength={280} aria-label="Group description" placeholder="Description (optional)" value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /><div className="modal-actions"><button type="submit" className="btn-primary">Create</button><button type="button" className="btn-secondary" onClick={() => setCreating(false)}>Cancel</button></div></form> : null}
